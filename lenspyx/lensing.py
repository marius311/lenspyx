...
def strip_alm2lenmap(alm, dlms, nside, theta1, theta2, nband=8, facres=0, verbose=True, symmetric_band=True):
    r"""Computes a deflected spin-0 healpix map from its alm and deflection field alm.
        ...
        Args:
            ...
            nside: desired healpix resolution of the deflected map
            theta1, theta2: The range of the strip 
                            These theta's are defined on the southern hemisphere for spt3g project
                    If theta1 < theta2, the strip is [theta1, theta2]
                    if theta1 > theta2, the strip is [theta2, theta1]
            facres(optional): the deflected map is constructed by interpolation of the undeflected map,
                              built at target res. ~ :math:`0.7 * 2^{\rm facres}.` arcmin
            nband(optional): To avoid dealing with too many large maps in memory, the operations is split in bands.
            verbose(optional): If set, prints a bunch of timing and other info. Defaults to true.
            symmetric_band(optional): If set True, do lensing on the symmetrical bands on north and south hemisphere
                                      If set False, only do strip lensing on one hemisphere
    ...
    assert len(dlms) == 2
    if not np.iscomplexobj(dlms[0]): assert dlms[0].size == dlms[1].size and dlms[0].size == hp.nside2npix(nside)
    return _strip_lens_gclm_sym_timed(0, dlms[0], -alm, nside, theta1, theta2, dclm=dlms[1], nband=nband, facres=facres, verbose=verbose, symmetric_band=symmetric_band)


def strip_alm2lenmap_spin(gclm, dlms, nside, spin, theta1, theta2, nband=4, facres=-1, verbose=True, symmetric_band=True):
    r"""Computes a deflected spin-weight Healpix map from its gradient and curl modes and deflection field alm.
    ...
    ret = _strip_lens_gclm_sym_timed(spin, dlms[0], gclm[0], nside, theta1, theta2,
                                clm=gclm[1], dclm=dlms[1], nband=nband, facres=facres, verbose=verbose, symmetric_band=symmetric_band)
    return ret.real, ret.imag
...
def _lens_gclm_sym_timed(spin, dlm, glm, nside, nband=8, facres=0, clm=None, dclm=None, verbose=True):
    ...
    target_nt = 3 ** 1 * 2 ** (12 + facres) # on one hemisphere
...
### Modified by FG
def _strip_lens_gclm_sym_timed(spin, dlm, glm, nside, theta1, theta2, nband=4, facres=0, clm=None, dclm=None, verbose=True, symmetric_band=True):
    """Performs the deflection by splitting the full latitude range into distinct bands which are done one at a time.

        See *_lens_gcband_sym* for the single band (north and south hemispheres) lensing.

        symmetric_band: Bool
            If True, the lensing will be performed in the symmetric bands about the equator
            If False, only the band in the southern hemisphere covering the SPT3G area will be lensed
...
#    for ib in range(1):
        ...
        ## The northern hemisphere is also included to augment the number of maps of 3g patch generated from one lensing operation
        pixn = hp.query_strip(nside, np.pi - th2, np.pi - th1, inclusive=True)
        pixs = hp.query_strip(nside, th1, th2, inclusive=True)
        thtpn, phipn = angles.get_angles(nside, pixn, redtot[pixn], imdtot[pixn], 'north', verbose=verbose)
        thtps, phips = angles.get_angles(nside, pixs, redtot[pixs], imdtot[pixs], 'south', verbose=verbose)
        ...
        buffS = 10 * (mathtps - mithtps) / (nt_perband - 1) / (1. - 2. * 10. / (nt_perband - 1))
        if (symmetric_band):
            th1 = min(np.pi - (mathtps + buffS),mithtpn - buffN)
            th2 = max(np.pi - (mithtps - buffS),mathtpn + buffN)
        else:
            th1 = mithtps - buffS
            th2 = mathtps + buffS
        ...
        if (symmetric_band):
            len_nr, len_ni, len_sr, len_si = _lens_gcband_sym(spin, glm, th1, th2, nt_perband, nphi, thtpn, phipn, thtps, phips,
                                                              clm=clm, times=times)
            if spin == 0:
                ret[pixn] = len_nr
                ret[pixs] = len_sr
            ...
            len_sr = bicubic.deflect(np.require(filtmap.real, np.float64, requirements='F'), t_grid, p_grid)
            len_si = bicubic.deflect(np.require(filtmap.imag, np.float64, requirements='F'), t_grid, p_grid) if spin > 0 else None
            times.add('interp')
